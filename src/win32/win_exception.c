...
static void WIN_CreateDumpFilePath(char* buffer, const char* fileName, SYSTEMTIME* time)
{
	...
	const char* reportPath = va("%s\\rtcwpro_crash", tempPath);
	if (tempPath == NULL || !WIN_CreateDirectoryIfNeeded(reportPath)) {
		reportPath = ".rtcwpro_crash";
...
static const char* WIN_GetExceptionCodeString(DWORD exceptionCode)
{
	...
	case RTCWPRO_WINDOWS_EXCEPTION_CODE: return "RtcwPro Exception";
...
static qbool WIN_WriteTextData(const char* filePath, debug_help_t* debugHelp, EXCEPTION_RECORD* pExceptionRecord)
{
	...
	if (pExceptionRecord->ExceptionCode == RTCWPRO_WINDOWS_EXCEPTION_CODE &&
	...
	//Crash_PrintToFile(__argv[0]); //QVM dump
...
{
	//CNQ3 multi-monitor support not implemented
...
static LONG WIN_HandleCrash(EXCEPTION_POINTERS* ep)
{
	...
	static const char* mbTitle = "RtcwPro Crash";
	static const char* mbMsg = "RtcwPro crashed!\n\nYes to generate a crash report\nNo to continue after attaching a debugger\nCancel to quit";
	...
	if (result == IDYES) {
		...
		if (!exc_quietMode) {
			if (exc_reportWritten)
				...
				MessageBoxA(NULL, "RtcwPro's crash report generation failed!\nExiting now", mbTitle, MB_OK | MB_ICONERROR);
		}
	}
...
static void WIN_RaiseException_f(void)
{

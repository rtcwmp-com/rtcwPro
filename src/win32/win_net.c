...
void BogusFailureCallback(char* errmsg)
{
	Com_Printf("BogusFailureCallback:%s\n", errmsg);
}
...
void NET_CloseStreamedSocket(streamed_socket* ss)
{
	...
	ss->in_use = qfalse;
	ss->socket = -1;
	ss->failure_callback = BogusFailureCallback;
}
...
qboolean NET_OpenStreamedSocket(streamed_socket** ss_out, netadr_t* to)
{
	...
	if ((ss->socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET)
	{
	...
	if (setsockopt(ss->socket, SOL_SOCKET, SO_KEEPALIVE, &keep_alive, sizeof(keep_alive)) != 0)
	{
	...
	if (ioctlsocket(ss->socket, FIONBIO, &non_blocking) == SOCKET_ERROR)
	{
	...
	if (connect(ss->socket, &addr, sizeof(addr)) == SOCKET_ERROR)
	{
		err = WSAGetLastError();
		if (err != WSAEWOULDBLOCK)
		{
...
void Sys_SendStreamedPacket(streamed_socket* ss, void* data, int length)
{
	...
	if (send(ss->socket, data, length, 0) == SOCKET_ERROR)
	{
		err = WSAGetLastError();
		if (err == WSAENOTCONN)
		{
			QueueStreamedPacket(ss, data, length, 1000, qfalse, 2);
		}
		else if (err != WSAEWOULDBLOCK)
		{
			ss->failure_callback(NET_ErrorString());
			NET_CloseStreamedSocket(ss);
...
void Sys_ResendStreamedPacket(void)
{
	...
	if (send(streamed_packet->ss->socket, streamed_packet->data, streamed_packet->data_length, 0) == SOCKET_ERROR)
	{
		err = WSAGetLastError();
		if (err == WSAENOTCONN)
		{
			...
			{
				streamed_packet->ss->failure_callback(va("Sys_ResendStreamedPacket send: Maximum retry attempts of %i reached.", streamed_packet->max_attempts));
				FreeQueuedStreamedPacket(streamed_packet, qtrue);
			}
			else
			{
		...
		{
			FreeQueuedStreamedPacket(streamed_packet, qtrue);
			streamed_packet->ss->failure_callback(NET_ErrorString());
...
qboolean Sys_GetStreamedPacket(netadr_t* net_from, msg_t* msg)
{
	...
	for (i = 0; i < MAX_STREAMED_SOCKETS; ++i)
	{
		...
		{
			// connection has been closed server-side (keepalive timeout)
			NET_CloseStreamedSocket(ss);
			continue;
		}
